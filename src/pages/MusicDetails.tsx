      ...
      <div className="flex items-center gap-x-5 text-4xl px-6 my-6">
        <button className="bg-[#1ECF5D] p-3 rounded-full text-black hover:scale-105"
          ...
          {songId === currentMusic?.id ? (
            <BsFillPauseFill size={30} /> // Show pause button if the song is playing
          ) : (
            <BsFillPlayFill size={30} /> // Show play button if the song is not playing
          )}
        </button>
        <button>
          <AiOutlineHeart />
      ...
      <div className="min-h-[70vh] px-6">
        <ul className="px-4">
          ...
          {MusicData.map((item, i) => (
            <li
              key={i}
              className="flex items-center justify-between pb-1 text-white"
              onMouseOver={() => onHoverSong(item.id)}
            >
              <div className={`flex items-center gap-x-3 ${songId === item.id ? "text-[#1ECF5D]" : "text-white"}`}>
                {songId === item.id && (
                  <button
                    onClick={() => stopSong()}
                    className="focus:outline-none"
                  >
                    {songId === item.id && hoveredSong === item.id? (
                ...
                {songId !== item.id && (
                  <span className="w-[25px] pl-2">{i + 1}</span>
                )}
